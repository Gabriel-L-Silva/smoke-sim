...
func _draw():
	...
	var limit_x = grid.squares_qtd.x * grid.cell_size.x
	var limit_y = grid.squares_qtd.y * grid.cell_size.y

	# cols
	for x in range(grid.squares_qtd.x+1):
		var col_pos = x * grid.cell_size.x
		draw_line(Vector2(col_pos, 0), 
				  Vector2(col_pos, limit_y), LINE_COLOR, LINE_WIDTH, true)

	# rows
	for y in range(grid.squares_qtd.y+1):
		var row_pos = y * grid.cell_size.y + LINE_WIDTH
